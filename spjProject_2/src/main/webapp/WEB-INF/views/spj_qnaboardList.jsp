...
<div id="main" class="board-table mt-5">
	...
	<span class="bt_wrap">
		<a href="spj_QnAboardWrite" class="btn btn-dark mb-2">게시글 작성</a>
		<!--<a href="#">수정</a>-->
	</span>
	...
	<table id="searchTable" class="board_list">
		...
		<tbody>
			<c:forEach items="${qlistContent}" var="qdto">
				<tr>
					<td class="num" id="qid">${qdto.qId}</td>
					<td class="title">
						<a class="spj_QnaboardView" href="spj_QnaboardView?qId=${qdto.qId}">${qdto.qTitle}</a>
					</td>
					<td class="writer">${qdto.qNic}</td>
					<td class="date">${qdto.qDate}</td>
					<td class="count">${qdto.qHit}</td>
				</tr>
			</c:forEach>
		</tbody>			
	</table>
...
$(document).ready(function(){
	...
	$("#write").click(function(event){
		...
		$.ajax({
			...
			error : function() {
				alert("spj_QnAboardWrite시 에러 발생");
			}
		});
	});
	
	//내용보기
	$(".spj_QnaboardView").click(function(event){
		$.ajax({
			url : x,
			type : "get", //a의 방식은  get
			success : function(data) {
